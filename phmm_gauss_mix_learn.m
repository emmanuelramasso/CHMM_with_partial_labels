...
for essai=1:nessai,
        ...
        elseif 0
            ...
            for i=1:25
                ...
                try
                    mix = gmm(d,K*M,'full');
                    optionsGMM = foptions;
                    optionsGMM(14) = 3; % nb iterations
                    mix = gmminit(mix, x, optionsGMM);
                    mix.pl = pl;
                    mix = gmmem_pl(mix, x, optionsGMM);
                    %post = gmmpost_pl(mix, x);
                    %[a, b]=max(post,[],2);
                    mixmat = reshape(mix.priors,K,M);
                    Sig = reshape(mix.covars,[d, d, K, M]);
            ...
            [mu0, Sigma0] = mixgauss_init(K*M, x', 'full');
            mu0 = reshape(mu0, [d K M]);
            Sigma0 = reshape(Sigma0, [d d K M]);
            mixmat0 = mk_stochastic(rand(K,M));
            [LL, Pi, A, mu, Sig, mixmat] = ...
                mhmm_em(x', prior0, transmat0, mu0, Sigma0, mixmat0, 'max_iter', 500);
    ...
    while go_on,
        ...
        try
            ...
            %[p, p2] = computeB(x, mu, Sig, mixmat, K, M, T); %B2(i,k,t) = Pr(y(t) | Q(t)=i, M(t)=k)
            [p, p2] = mixgauss_prob(x', reshape(mu,[d,K,M]), Sig, mixmat); p=p'; p2=permute(p2,[3,1,2]);
            ...
            if (q>nitermax) || ((q>1) && ((abs(logL(q)-logL(q-1))/abs(logL(q-1)) < thresh))),
                ...
                if logL(q) > logLmax && isfinite(logL(q))
        ...
        if 1%parametersAlgorithm.visu
            disp(sprintf('PHMM Iteration %d -> Loglik=%f',q,logL(q)))
        end
